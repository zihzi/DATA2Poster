    ...
    if api_keys_entered:
        ...
        # Second stage to score related facts based on the selected question
            ...
            for query in q_for_nl4DV:
                ...
                try:
                    code_executed = preprocess_json(chart_code.content, idx)
                    with open(f"DATA2Poster_json/vega_lite_json_{idx}.json", "r") as f:
                        chart_viscorpus = json.load(f)
                        st.vega_lite_chart(chart_viscorpus, theme = None)
                except Exception as e:
                    st.code(chart_code.content)
                    st.write("Error:",str(e))
                st.subheader("NL4DV Chart:")
                # Call GPT to generate vis code
                print("\n🟢 Step 1: Generating Initial Code...\n")
                prompt_nl4DV = load_prompt_from_file("prompt_templates/nl4DV_prompt.txt")
                ...
                initial_code = agent_1_generate_code(query, datasets[chosen_dataset], nl4DV_json, code_template, openai_key)
                print(initial_code)
                try:
                    code_executed_nl4DV = preprocess_json_2(initial_code, idx)
                    with open(f"nl4DV_json/vega_lite_json_{idx}.json", "r") as f:
                        chart_nl4dv = json.load(f)
                        st.vega_lite_chart(chart_nl4dv, theme = None)
                except Exception as e:
                    st.code(initial_code)
                    st.write("Error:",str(e))
