...
def agent6_vis_generator(table_name, query,title, data_schema,sampled_data, vlspec, openai_key):
    ...
    - If "mark": {{ "type": "arc" }} is used in the Vega-Lite specification, make sure **no "legend: null"** operation in the encoding property.
    - If "mark": {{ "type": "arc"}} is used in the Vega-Lite specification, make sure **"color": {{"field": "...","type": "nominal","legend": {{"title": "..."}},"scale": {{"scheme": "..."}}}}** operation in the encoding property.
...
def agent6_sec_vis_generator(table_name, query,title, data_schema,sampled_data, vlspec, openai_key):
    prompt = """
    You are a data analysis assistant that uses Vega-Lite to create data visualizations.
    Your task is to create optimal visualizations for this query {query} based on the {table_name} data table using Vega-Lite.
    The visualizations title for the chart should be the following title:{title}
    The data content overview of the {table_name} table is as follows:{data_schema}
    The {table_name} sample data table is as follows:{sampled_data}
    Here is the Vega-Lite specification suitable for these queries for your reference:{vlspec}
...
def agent9_section_designer(img_list, insight_list, openai_key):
    ...
    section_prompt ="""
                            ...
                            Each section contains 2 charts.
...
def poster_title(intro,conclusion,openai_key):
    ...
    prompt = PromptTemplate(
            template="""
            You are an excellent data analyst writing title for a data visualization poster.
